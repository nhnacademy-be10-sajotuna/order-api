...
public class AvailableCouponController {
    ...
    // 사용 가능한 북 쿠폰 조회
    @GetMapping("/book")
    public ResponseEntity<List<CouponResponse>> getAvailableBookCoupons(@RequestHeader("X-User-Id") Long userId, @ModelAttribute BookInfo bookInfo) {
    ...
    // 사용 가능한 오더 쿠폰 조회
    @GetMapping("/order")
    public ResponseEntity<List<CouponResponse>> getAvailableOrderCoupons(@RequestHeader("X-User-Id") Long userId, @RequestParam Integer totalPrice){
